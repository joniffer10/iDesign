<template>
  <span :class="['id-badge', `badge-${variant}`, { 'has-count': hasCount }]">
    <slot />
    <span v-if="hasCount" class="badge-count">{{ displayCount }}</span>
    <span v-else-if="dot && showBadge" class="badge-dot" />
  </span>
</template>
