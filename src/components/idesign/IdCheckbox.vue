<template>
  <label :class="['id-checkbox', `size-${size}`, `color-${color}`, { 'is-checked': modelValue, 'is-disabled': disabled }]">
    <input type="checkbox" :checked="modelValue" :disabled="disabled" class="sr-only" @change="$emit('update:modelValue', $event.target.checked)" />
    <span :class="['checkbox-box', { checked: modelValue }]" role="checkbox" :aria-checked="modelValue">
      <svg v-if="modelValue" :width="checkSize" :height="checkSize" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="3" stroke-linecap="round"><path d="M20 6L9 17l-5-5"/></svg>
    </span>
